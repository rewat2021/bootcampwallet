<template>
  <CenterMain>
    ...
    <div
      v-if="groupedHistoryElements.size > 0"
      aria-label="Credential list"
      class="h-full overflow-y-auto shadow-xl"
    >
      <div
        v-for="group in groupedHistoryElements.keys()"
        :key="group.id"
        class="relative"
      >
        <div
        ...
        <ul class="divide-y divide-gray-100 overflow-x-clip" role="list">
          <li
            v-for="element in groupedHistoryElements.get(group)"
            :key="element"
          ...
          >
            ...
            <div class="min-w-0">
              <div class="flex items-start gap-x-3">
                ...
                <div
                  v-else-if="
                    element.entry.operation === 'usePresentationRequest'
                  "
                >
                  <p class="text-sm font-semibold leading-6 text-gray-900">
                    Used presentation request
                  </p>
                  <p class="text-xs overflow-x-scroll w-1/3 h-5">
                    <code>{{ JSON.stringify(element.entry.data) }}</code>
                  </p>
    ...
    <div v-else class="mt-2 border-l pl-2">
      ...
      <p class="text-base text-gray-500">
        No operations have been logged to your auditable history yet.
      </p>
    </div>
  </CenterMain>
</template>
...
const {
  ...
  refresh,
  error,
} = await useLazyFetch(`/wallet-api/wallet/${currentWallet.value}/history`);
refreshNuxtData();
