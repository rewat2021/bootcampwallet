<template>
  <CenterMain>
    ...
    <div v-if="exportedKey" class="my-2">
      <span v-if="loadPrivateKey" class="font-bold text-red-600"
        >NEVER SHARE YOUR PRIVATE KEY.</span
      >

      <div class="my-2">
        <pre v-if="format == 'JWK'">{{
          JSON.stringify(JSON.parse(exportedKey), null, 2)
        }}</pre>
        <pre v-else>{{ exportedKey }}</pre>
      </div>

      <qrcode-vue
        v-if="exportedKey.length <= 4296"
        :value="exportedKey"
        level="L"
        size="300"
      ></qrcode-vue>
      <p v-else>
        Unfortunately, this Verifiable Credential is too big to be viewable as
        QR code (credential size is {{ exportedKey.length }} characters, but the
...
async function loadExportedKey() {
  const data = await $fetch(
    `/wallet-api/wallet/${currentWallet.value}/keys/${keyId}/export?format=${format}&loadPrivateKey=${loadPrivateKey}`,
  );
