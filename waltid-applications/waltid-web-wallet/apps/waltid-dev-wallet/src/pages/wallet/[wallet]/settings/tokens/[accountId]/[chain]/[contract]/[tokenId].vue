  ...
  <CenterMain>
    ...
    <div v-else>
      ...
      <div>
        ...
        <div v-if="isRedeemable">
          ...
          >
            ...
            ><span v-if="showQR">Close QR</span
            ><span v-if="!showQR">Show QR</span>
          </button>
        </div>
      </div>
      <!-- token properties -->
      <div class="text-left pt-2">
        <div
          v-if="isRedeemable && isRedeemed"
          class="alert alert-info mt-3 mx-3"
        >
      ...
      <div v-if="isNotNullOrEmpty(explorer?.url)">
        <a
          ...
          target="_blank"
        >
          <span class="ml-1 text-gray-800 font-semibold h-5"
            >View on blockchain explorer</span
          >
        </a>
      </div>
...
const {
  ...
  `/wallet-api/wallet/${currentWallet.value}/nft/detail/${accountId}/${chain.toUpperCase()}/${contract}/${tokenId}${collectionId ? "?collectionId=" + collectionId : ""}`,
);
const { data: marketplace } = useLazyFetch(
  `/wallet-api/wallet/${currentWallet.value}/nft/marketplace/${chain}/${contract}/${tokenId}`,
);
const { data: explorer } = useLazyFetch(
  `/wallet-api/wallet/${currentWallet.value}/nft/explorer/${chain}/${contract}`,
);
refreshNuxtData();
...
function isRedeemable() {
  return (
    !showRedeem &&
    nft.attributes &&
    nft.attributes.find((a) => a.traitType == "redeemed") != null
  );
}
...
function isRedeemed() {
  return (
    nft.attributes.find(
      (a) => a.traitType == "redeemed" && a.value == "true",
    ) != null
  );
}
