...
export default defineEventHandler(async (event) => {
    ...
    if (name == "list" || name === undefined) {
        ...
        return contentQuery.filter((page) => page._path?.startsWith("/sd-jwt-credentials/") && page.body?.children.find((elem) => elem.tag === "pre" && elem.props?.language === "json") != undefined).map((elem) => elem.title);
    }

    const contentQuery = await serverQueryContent(event).find();

    const matchedContent = contentQuery.find((content) => {
        return content._path?.startsWith("/sd-jwt-credentials/") && content.title === name;
    });
