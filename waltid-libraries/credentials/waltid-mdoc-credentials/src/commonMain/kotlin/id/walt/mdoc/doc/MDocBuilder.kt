...
class MDocBuilder(val docType: String) {
    ...
    fun build(issuerAuth: COSESign1?, deviceSigned: DeviceSigned? = null): MDoc {
        return MDoc(
            ...
            IssuerSigned(
                nameSpacesMap.mapValues { it.value.map { item -> EncodedCBORElement(item.toMapElement()) } },
                issuerAuth
            ),
            deviceSigned
        )
    ...
    suspend fun signAsync(
        ...
        cryptoProvider: AsyncCOSECryptoProvider,
        keyID: String? = null,
        status: Status? = null,
    ): MDoc {
        val mso = MSO.createFor(nameSpacesMap, deviceKeyInfo, docType, validityInfo, status = status)
