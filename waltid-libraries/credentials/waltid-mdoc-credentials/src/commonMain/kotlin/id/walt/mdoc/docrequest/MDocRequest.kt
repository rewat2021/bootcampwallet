...
data class MDocRequest internal constructor(
    ...
    private fun getReaderSignedPayload(readerAuthentication: ReaderAuthentication) =
        EncodedCBORElement(readerAuthentication.toDE()).toCBOR()
        ...
        return (!verificationParams.requiresReaderAuth ||
                readerAuth?.let {
                    val readerAuthentication =
                        verificationParams.readerAuthentication?.let { getReaderSignedPayload(it) }
                            ?: throw Exception("No reader authentication payload given")
