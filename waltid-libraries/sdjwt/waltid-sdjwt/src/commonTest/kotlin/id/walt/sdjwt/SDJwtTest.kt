...
class SDJwtTest {
    ...
    fun testSDPayloadGeneration() {
        ...
        assertEquals(
            expected = sdPayload_1.digestedDisclosures.size,
            actual = sdPayload_1.undisclosedPayload[SDJwt.DIGESTS_KEY]!!.jsonArray.size
        )
        ...
        assertEquals(
            expected = sdPayload_2.digestedDisclosures.size, actual = sdPayload_2.undisclosedPayload[SDJwt.DIGESTS_KEY]!!.jsonArray.size +
        ...
        assertEquals(
            expected = sdPayload_3.digestedDisclosures.size, actual = sdPayload_3.undisclosedPayload[SDJwt.DIGESTS_KEY]!!.jsonArray.size +
    ...
    fun testParseKeyBindingJwt() {
        ...
        assertEquals(sdHash, SHA256().digest(ASCII.encode(issuerJwt)).encodeToBase64Url())
    }
}
