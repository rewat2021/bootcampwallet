...
/*
 * Adapted from https://github.com/sqids/sqids-kotlin (MIT License - 2023-present Sqids maintainers.)
 */
...
class Sqids(
    ...
    private var blockList: Set<String> = DEFAULT_BLOCK_LIST
) {
    init {
        if (alphabet.length != alphabet.encodeToByteArray().size) {
    ...
    private fun toNumber(id: String, alphabet: String): Long = id.fold(0) { number, c ->
        number * alphabet.length + alphabet.indexOf(c)
    }
...
/*
 * We first calculate the integer division x / y, which gives us the quotient.
 * Then, we calculate the remainder using the modulo operator %.
 * If the remainder is non-zero and the signs of x and y differ, it means the
 *   division should round towards negative infinity, so we subtract 1 from the quotient.
 * Otherwise, we return the quotient as is.
 */
private fun floorDiv(x: Long, y: Int): Long {
    val div = x / y
    val mod = x % y

    return if ((mod != 0.toLong()) && ((x < 0) xor (y < 0))) div - 1 else div
}
