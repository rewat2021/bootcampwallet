...
abstract class OpenIDCredentialWallet<S : SIOPSession>(
    ...
    return ((authorizationRequest.responseType.contains(ResponseType.VpToken) ||
            authorizationRequest.responseType.contains(ResponseType.IdToken)) &&
            (authorizationRequest.presentationDefinition != null || authorizationRequest.customParameters.containsKey("dcql_query")) &&
            (authorizationRequest.presentationDefinition == null ||
             isPresentationDefinitionSupported(authorizationRequest.presentationDefinition))
            )
}
    ...
    try {
        return authorizationRequest.copy(
            presentationDefinition = authorizationRequest.presentationDefinition
				...
				?: run {
    println("DEBUG keys: ${authorizationRequest.customParameters.keys}")
    authorizationRequest.customParameters["dcql_query"]?.firstOrNull()?.let {
        PresentationDefinition(
            id = "dcql_presentation",
            inputDescriptors = listOf(InputDescriptor(id = "dcql_credential"))
        )
    }
}
                ?: authorizationRequest.customParameters["dcql_query"]?.firstOrNull()?.let {
    PresentationDefinition(
        ...
        inputDescriptors = listOf(
            InputDescriptor(id = "dcql_credential",
    ...
    override fun generateTokenResponse(session: S, tokenRequest: TokenRequest): TokenResponse {
		...
		if (session.authorizationRequest?.presentationDefinition == null && 
    session.authorizationRequest?.customParameters?.containsKey("dcql_query") != true)  {
				throw TokenError(
				tokenRequest = tokenRequest,
				errorCode = TokenErrorCode.invalid_request
			)
