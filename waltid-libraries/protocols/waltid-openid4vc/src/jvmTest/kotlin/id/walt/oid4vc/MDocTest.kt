...
//            jwks Authorization Request parameter can be used for key agreement to encrypt the response (see [7]). [ISO-18013-7_240312]
                        put("keys", JsonArray(listOf(runBlocking {
                            ephemeralReaderKey.getPublicKey().exportJWKObject().let {
        ...
        val encKey = parsedPresReq.clientMetadata.jwks["keys"]?.jsonArray?.first { jwk ->
            JWK.parse(jwk.toString()).keyUse?.equals(KeyUse.ENCRYPTION) ?: false
        }?.jsonObject ?: throw Exception("No ephemeral reader key found")

        val formParams = oid4vpResponse.toDirectPostJWTParameters(
            encKey,
        ...
        val mdocHandoverRestored = OpenID4VP.generateMDocOID4VPHandover(
            presReq,
            Base64URL.from(parsedResponse.jwsParts.header["apu"]!!.jsonPrimitive.content).decodeToString()
        )
        ...
        val verified = parsedMdoc.verify(
