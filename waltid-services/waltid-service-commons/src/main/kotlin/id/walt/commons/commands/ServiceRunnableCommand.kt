...
class ServiceRunnableCommand(
    ...
    private val logLevel: String? by option(
        ...
        "-l",
        envvar = "LOG_LEVEL"
    ).choice(*LoggingManager.loggingSetups.keys.toTypedArray(), ignoreCase = true)
    private val logType: RenderStrings? by option("--log-type", "--logType", envvar = "LOG_TYPE").enum<RenderStrings>(ignoreCase = true)
