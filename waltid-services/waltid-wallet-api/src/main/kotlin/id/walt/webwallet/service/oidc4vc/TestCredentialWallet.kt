...
class TestCredentialWallet(
    ...
    override fun generatePresentationForVPToken(
		...
		*/
		val sessionKey = (session.authorizationRequest!!.state ?: "") +
			(session.authorizationRequest.presentationDefinition?.id ?: "dcql")
		
		logger.info("=== sessionKey: {sessionKey}", sessionKey)
		logger.info("=== available keys: {keys}", HACK_outsideMappedSelectedCredentialsPerSession.keys.toString())

		val selectedCredentials =
			HACK_outsideMappedSelectedCredentialsPerSession[sessionKey]
				?: HACK_outsideMappedSelectedCredentialsPerSession.entries
					.firstOrNull { it.key.startsWith(session.authorizationRequest!!.state ?: "") }
						?.value
				?: emptyList()

		val selectedDisclosures =
			HACK_outsideMappedSelectedDisclosuresPerSession[sessionKey]
				?: HACK_outsideMappedSelectedDisclosuresPerSession.entries
					.firstOrNull { it.key.startsWith(session.authorizationRequest!!.state ?: "") }
					?.value

        logger.info("Selected credentials: {selectedCredentials}", selectedCredentials)
        val matchedCredentials = credentialsService.get(wallet, selectedCredentials)
        logger.info("Matched credentials: {matchedCredentials}", matchedCredentials)

        logger.info("Using disclosures: {selectedDisclosures}", selectedDisclosures)
        val key = runBlocking {
            runCatching {
        ...
        val sdJwtVCsPresented = runBlocking {
            matchedCredentials.filter { it.format == CredentialFormat.sd_jwt_vc || it.format == CredentialFormat.sd_jwt_dc }.map {
                // TODO: adopt selective disclosure selection (doesn't work with jwts other than sd_jwt anyway, like above)
                /*val disclosureStr = if (selectedDisclosures?.containsKey(it.id) == true) {
						selectedDisclosures[it.id]!!.joinToString("~")
					} else {
						it.disclosures ?: ""
					}*/
				//val disclosureStr = it.disclosures ?: ""
				val disclosureStr = if (selectedDisclosures?.containsKey(it.id) == true && it.disclosures != null) {
				val selectedClaimNames = selectedDisclosures[it.id]!!.mapNotNull { sel ->
				try {
						// ลอง standard base64 ก่อน (UI ส่งมาแบบนี้)
						val decoded = try {
										java.util.Base64.getDecoder().decode(
											sel + "=".repeat((4 - sel.length % 4) % 4)
										)
						} catch (e: Exception) {
								// fallback urlsafe
						...
						val arr = kotlinx.serialization.json.Json.parseToJsonElement(String(decoded, Charsets.UTF_8))
							.jsonArray
							arr[1].jsonPrimitive.content
				} catch (e: Exception) {
							logger.info("sel decode failed: ${e.message}, sel=${sel.take(20)}")
							null
				}
				}.toSet()
						logger.info("Selected claim names: $selectedClaimNames")
					it.disclosures!!.split("~").filter { dbDisc ->
					if (dbDisc.isEmpty()) return@filter false
					try {
							val decoded = java.util.Base64.getUrlDecoder().decode(
								dbDisc + "=".repeat((4 - dbDisc.length % 4) % 4)
						)
							val claimName = kotlinx.serialization.json.Json.parseToJsonElement(String(decoded))
								.jsonArray[1].jsonPrimitive.content
							val matched = claimName in selectedClaimNames
						logger.info("DB claim: $claimName, matched: $matched")
						matched
				...
				val documentWithDisclosures = "${it.document}~${disclosureStr}~"
            ...
            presentations, PresentationSubmission(
                ...
                descriptorMap = matchedCredentials.mapIndexed { index, credential ->
                    when (credential.format) {
                        ...
                        CredentialFormat.sd_jwt_vc, CredentialFormat.sd_jwt_dc -> buildDescriptorMappingSDJwtVC(
                            session.presentationDefinition,
                            credential.document,
                            "$",
                        )
